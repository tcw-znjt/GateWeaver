...
// Cleanup 完全拆除（stop/uninstall 兜底）。同时幂等清除 v0.2 遗留的
// GW_CLASH_* 链与 fwmark 策略路由（升级兼容，见 switch-to-tun design D4）。
func (m *RuleMgr) Cleanup(ctx context.Context) error {
	...
	try("-t", "nat", "-D", "PREROUTING", "-j", "GW_CLASH_DNS")
	try("-t", "nat", "-D", "PREROUTING", "-j", "GW_CLASH_TCP")
	try("-t", "mangle", "-D", "PREROUTING", "-j", "GW_CLASH_UDP")
	for _, pair := range [][2]string{
		{"filter", fwdChain}, {"nat", natChain},
		{"nat", "GW_CLASH_DNS"}, {"nat", "GW_CLASH_TCP"}, {"mangle", "GW_CLASH_UDP"},
	} {
		try("-t", pair[0], "-F", pair[1])
		try("-t", pair[0], "-X", pair[1])
	}
	_, _ = m.r.Run(ctx, "sh", "-c", "while ip rule del fwmark 0x1/0x1 table 100 2>/dev/null; do :; done; true")
	_, _ = m.r.Run(ctx, "sh", "-c", "ip route del local default dev lo table 100 2>/dev/null; true")
